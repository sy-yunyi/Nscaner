...
def scanning_domain_zmap(domain):
    ...
    # cmd = f"cat /home/usertoor/silex/ghostR/data/zonefile_ns_records_domain_0531_uniq.txt | /home/usertoor/silex/zdns/zdns/zdns A -name-servers {ns}"
    # cmd = f"cat 'baidu.com' | /home/usertoor/silex/zdns/zdns/zdns A -name-servers {ns}"
    cmd = f"echo 'toortoor' |sudo -S zmap -p 53 -B 3M --probe-module=dns --probe-args='A,{domain}' -O json --output-fields=* --output-file=/home/usertoor/silex/domain_mirroring/data/zmap/{domain}-.res --interface=enp96s0f1 --list-of-ips-file=/home/usertoor/silex/domain_mirroring/data/name_server_ips.txt"
    p = subprocess.Popen(cmd,shell=True,stdin=subprocess.PIPE,stdout=subprocess.PIPE)
...
if __name__ == '__main__':
    ...
    # 使用zmap扫描
    print("zmap scanning ...")
    fp = open("/home/usertoor/silex/domain_mirroring/data/domains_100.txt","r")
    lines = fp.readlines()
    lines = [line.strip() for line in lines]
    # lines = ["baidu.com"]
